...
namespace CapaPresentacion
{
    public partial class MantenedorProforma : Form
    {
        ...
        // ── Ver detalle al seleccionar proforma ──────────────
        private void dgvProformas_Click(object sender, EventArgs e)
        {
            if (dgvProformas.CurrentRow == null) return;
            int id = Convert.ToInt32(dgvProformas.CurrentRow.Cells["IdProforma"].Value);

            var proforma = logP.ObtenerPorId(id);
            if (proforma == null) return;

            lblNombreCliente.Text = "Cliente: " + proforma.NombreCliente;
            lblCiudad.Text = "Ciudad: " + proforma.Ciudad;
            lblTotal.Text = "Total: S/" + proforma.Total.ToString("N2");
            detalles = proforma.Detalles;
            CargarGridDetalle();
            btnAnular.Enabled = proforma.Estado == "Pendiente";
        }
